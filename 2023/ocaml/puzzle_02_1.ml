...
let rec sum_valid_games accum = 
  ...
  | Some line -> 
    let (game_number, colors_str) = parse_line line in
    (* printf "Line: %s\n" line;
    printf "Game: %d\n" game_number;
    printf "Colors: %s\n" colors_str; *)
    ...
    printf "Max blue: %d\n" max_blue;
    printf "Max green: %d\n" max_green; *)
    if max_red <= 12 && max_blue <= 14 && max_green <= 13 then
      sum_valid_games (accum + game_number)
    else 
      sum_valid_games (accum)
  | None -> accum
...
let () = 
  ...
  (*
  dune build puzzle_02_1.exe && cat ../data/day-02-test | dune exec ./puzzle_02_1.exe  
  *)