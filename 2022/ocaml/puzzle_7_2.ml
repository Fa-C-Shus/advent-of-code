...
let process_leaves leaves acc filter accumulator =
    ...
    | leaf :: leaves ->
      let leaves = 
        ...
        | Some parent -> 
          ...
          if parent.todo == 0 && filter acc parent then
            parent :: leaves
          else 
            leaves
          in
...
let update_space_required = 30000000
let init_leaves = get_leaves data 
let space_used = process_leaves init_leaves 0 (fun _ _ -> true) (fun _ leaf -> leaf.size)
let space_needed = space_used - (disk_size - update_space_required)

let _ = printf "Disk size: %d\n\tRequired Space: %d\n\tTarget: %d\n" 
  disk_size 
  update_space_required 
  space_needed
let leaves = get_leaves data 

let answer = process_leaves 
  leaves space_used 
  (fun acc parent -> parent.size <= acc) 
  (fun acc leaf ->
    if leaf.size >= space_needed then
      min acc leaf.size
    else
      acc)
...
let () = printf "Answer: %d\n" answer
(*
dune build puzzle_7_2.exe && dune exec ./puzzle_7_2.exe   
*)  