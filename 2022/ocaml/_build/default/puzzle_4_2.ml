...
let rec play accum = 
  ...
  | Some line -> 
     ...
     ( match (elf1Schedule, elf2Schedule) with
      | _ when not (IntSet.disjoint elf1Schedule elf2Schedule) -> 
        (* printf "elf1 & elf2 have overlapping camps: %s\n" line; *)
        play (accum + 1)
      |_ -> play accum
     )
...
;;
  ...
  (*
  dune build puzzle_4_2.exe && dune exec ./puzzle_4_2.exe   
  dune build puzzle_4_2.exe && cat ../data/day-4-test | dune exec ./puzzle_4_2.exe   
  dune build puzzle_4_2.exe && cat ../data/day-4 | dune exec ./puzzle_4_2.exe   
  *)