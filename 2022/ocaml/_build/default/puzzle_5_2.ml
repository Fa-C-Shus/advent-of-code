...
(* The change here is that I move the crates to a temp stack then to the real stack *)
let rec process_moves stacks moves =
  ...
  | _ when List.length moves > 0 -> 
    ...
    (* move the crates to a temp stack *)
...
;;
  ...
  (*
  dune build puzzle_5_2.exe && dune exec ./puzzle_5_2.exe   
  *)